...
from config import (
    ...
    HEAD_PITCH_THRESHOLD, HEAD_YAW_THRESHOLD
)
...
# 3D canonical face model points (for head pose via solvePnP)
...
def _calc_ear(landmarks, indices, w, h):
...
def _calc_head_pose(landmarks, w, h):
    img_pts = np.array(
...
class DriverDetector:
    ...
    def process(self, frame):
        """
        Analyse a frame and return (annotated_frame, alert_active, alerts_list).
        alerts_list contains any of: 'DROWSY', 'YAWNING', 'DISTRACTED', 'NO FACE'
        """
        h, w = frame.shape[:2]
        ...
        if not results.multi_face_landmarks:
            ...
            # — EAR (drowsiness) —
            ear_val = (_calc_ear(lm, LEFT_EYE, w, h) + _calc_ear(lm, RIGHT_EYE, w, h)) / 2.0
        ...
        return frame, len(alerts) > 0, alerts
